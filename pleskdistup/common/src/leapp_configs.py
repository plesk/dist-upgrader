...
LEAPP_MAP_JSON_PATH = os.path.join(PATH_TO_CONFIGFILES, "repomap.json")
...
def adopt_repositories(
    repofile: str,
    ignore: typing.Optional[typing.List[str]] = None,
    do_adapt_repository: typing.Callable[[rpm.Repository], rpm.Repository] = get_adapted_repository,
) -> None:
    if ignore is None:
        ignore = []
    ...
    with open(repofile + ".next", "a") as dst:
        for repo in rpm.extract_repodata(repofile):
            ...
            new_repo = do_adapt_repository(repo)
            dst.write(repr(new_repo))

    shutil.move(repofile + ".next", repofile)


def _add_repository_mapping_to_json(mapjson_path: str,
                                    old_id: str, new_id: str,
                                    distro: str = "",
                                    source_major_version: str = "7",
                                    target_major_version: str = "8") -> None:
    if not os.path.exists(mapjson_path):
        # If the mapping file doesn't exist, we need to create it with the basic structure
        mapping_json = {
        ...
        with open(mapjson_path, "r") as mapjson_file:
            mapping_json = json.load(mapjson_file)

    _add_repository_mapping_entry(mapping_json, old_id, new_id, distro,
                                  source_major_version, target_major_version)

    log.debug(f"Write json into '{mapjson_path}'")
    files.rewrite_json_file(mapjson_path, mapping_json)
    ...
    with open(leapp_repos_file_path, "a") as leapp_repos_file, open(mapfile_path, "a") as map_file:
        for file in repofiles:
            ...
            for repo in rpm.extract_repodata(file):
                ...
                try:
                    after_repository = do_adapt_repository(repo)
                    leapp_repos_file.write(repr(after_repository))
                ...
                if repo.id.startswith("PLESK_18_0") and "extras" in repo.id and repo.name is not None and repo.url is not None:
                    ...
                    after_dist_repository = do_adapt_repository(dist_repository_description)
                    leapp_repos_file.write(repr(after_dist_repository))
...
def add_repositories_mapping_json(repofiles: typing.List[str],
    ...
    with open(leapp_repos_file_path, "a") as leapp_repos_file:
        for file in repofiles:
            log.debug("Processing repofile '{filename}' into leapp configuration".format(filename=file))
            ...
            for repo in rpm.extract_repodata(file):
                ...
                leapp_repos_file.write("\n")
                if mapjson_path:
                    _add_repository_mapping_to_json(mapjson_path,
                                                    repo.id, after_repository.id,
                                                    distro, source_major_version, target_major_version)
...
def rm_package_from_pes_events(package: str,
                               repository: typing.Optional[str] = None,
                               action: typing.Optional[LeappActionType] = None,
                               leapp_pkgs_conf_path: str = LEAPP_PKGS_CONF_PATH,
                               ) -> None:
    pkg_mapping = None
    log.debug(f"Try remove package '{package}' from pes-events.json")
    with open(leapp_pkgs_conf_path, "r") as pkg_mapping_file:
        pkg_mapping = json.load(pkg_mapping_file)
        if not pkg_mapping["packageinfo"]:
            return
        for info in pkg_mapping["packageinfo"]:
            if action is not None and info["action"] != action:
                continue
            if not info["in_packageset"] or not info["in_packageset"]["package"]:
                continue
            for inpkg in info["in_packageset"]["package"]:
                if repository is not None and inpkg["repository"] != repository:
                    continue
                if inpkg["name"] == package:
                    log.debug(f"Remove '{package}' package from repository '{inpkg['repository']}' from info '{info['id']}' -> in packageset '{info['in_packageset']['set_id']}'")
                    info["in_packageset"]["package"].remove(inpkg)
...
def _add_repository_mapping_entry(dst: dict, id: typing.Optional[str],
                                  new_id: typing.Optional[str],
                                  distro: str = "",
                                  source_major_version: str = "7",
                                  target_major_version: str = "8") -> None:
    """
    ...
    src_entry = {
        "major_version": source_major_version,
        "repoid": id,
        "arch": "x86_64",
        "channel": "ga",
        "repo_type": "rpm"
    ...
    if distro:
        src_entry["distro"] = distro
        dst_entry["distro"] = distro
...
def create_leapp_vendor_repository_adoption(
    ...
    with open(target_repo_file + ".next", "w") as dst:
        for repo in rpm.extract_repodata(repofile):
            ...
            new_repo = do_adapt_repository(repo)
            dst.write(repr(new_repo))
            _add_repository_mapping_entry(mapping_json, repo.id,
                                          new_repo.id,
                                          distro=distro,
                                          source_major_version=source_major_version,
                                          target_major_version=target_major_version)
